#include "CameraCudaD3D11.h"
#include "cudaKernels.cuh" // For ApplyCudaKernel(...), though it does nothing now
...
// Forward-decl local helpers
static HRESULT ConfigureWindowAndD3D(int desiredW, int desiredH);
// We'll call ApplyCudaKernel(...) from cudaKernels.cuh, but it does nothing
        ...
        if (pSample) {
            ...
            if (SUCCEEDED(pSample->ConvertToContiguousBuffer(&pBuf)) && pBuf) {
                ...
                if (SUCCEEDED(pBuf->Lock(&pData, &maxLen, &curLen))) {
                    ...
                    if (!savedFrame) {
                        SaveRGB32AsBMP("debug_frame_noeffects.bmp", g_cpuBuffer.data(), g_frameWidth, g_frameHeight);
                        savedFrame = true;
                    }
...
static void NoOpCUDA()
{
    // If we do absolutely nothing, you can remove the entire CUDA registration.
    // But let's show how you'd "map/unmap" for zero effect:
    if (!g_pCudaGraphicsResource) return;

    // This is how you'd do a no-op map/unmap:
    cudaGraphicsMapResources(1, &g_pCudaGraphicsResource, 0);
    cudaGraphicsUnmapResources(1, &g_pCudaGraphicsResource, 0);
}

//--------------------------------------------------------------------------------------
// CreateSimpleShaders: PASS‐THROUGH in the pixel shader (NO channel swap, NO invert)
//--------------------------------------------------------------------------------------
void CreateSimpleShaders()
{
    ...
    // Pixel Shader: pass‐through (no channel swap)
    const char* psSrc = R"(
    ...
    {
        // Return the camera sample EXACTLY as is.
